...
## 更新摘要
**变更内容**
- 新增一票否决机制，设置天气和场景的门槛阈值
- 重构权重配置系统，调整核心权重分配
- 引入梯度推荐策略，提供最佳、替代和平衡三种方案
- 简化八字评分策略，采用更直观的评分模型
- 增强解释功能，提供更详细的推荐理由
...
**图表来源**
- [scorer.js](file://js/core/scorer.js#L1-L474)
- [scoring-config.js](file://js/core/scoring-config.js#L1-L127)
- [engine.js](file://js/services/engine.js#L1-L461)
- [recommendation.js](file://js/services/recommendation.js#L1-L274)
- [weather.js](file://js/services/weather.js#L1-L340)
- [results.js](file://js/controllers/results.js#L1-L656)
- [explanation.js](file://js/services/explanation.js#L1-L300)
- [schemes.json](file://data/schemes.json)
- [bazi-templates.json](file://data/bazi-templates.json)
...
- [scoring-config.js](file://js/core/scoring-config.js#L79-L91)
...
**更新** 新增一票否决机制，在计算总分前检查天气和场景的原始分数是否达到门槛阈值。
...
**章节来源**
- [scorer.js](file://js/core/scorer.js#L29-L474)

### 评分配置系统（scoring-config.js）
- 基础权重（SCORING_WEIGHTS.base）：节气 20%、八字 35%、场景 25%、天气 20%。
- 奖励权重（SCORING_WEIGHTS.bonus）：心愿 10%、历史偏好 5%、今日运势 5%。
- 门槛阈值（SCORING_WEIGHTS.threshold）：天气 30分、场景 40分。
- 动态权重（getDynamicWeights）：若无八字，则将 bazi 权重设为 0。
...
**更新** 调整了权重分配，增加了八字权重（35%），设置了门槛阈值用于一票否决。

```mermaid
flowchart TD
Start(["输入：用户画像"]) --> CheckBazi{"是否有八字？"}
CheckBazi -- 否 --> SetZero["将八字权重设为0"]
CheckBazi -- 是 --> Keep["保持原权重"]
SetZero --> NewUser{"是否新用户？"}
Keep --> NewUser
NewUser -- 是 --> Normal["保持权重不变"]
NewUser -- 否 --> Normal
Normal --> Done(["输出：动态权重"])
```
...
- 八字评分（scoreBazi）：简化策略 - 喜用神=100分（加权后+35），忌神=-57分（加权后-20），相生=57分（加权后+20），其他=0分。
...
**更新** 新增一票否决机制，天气和场景必须达到门槛阈值才能进入加权计算阶段。

```mermaid
flowchart TD
A["方案"] --> B["原始分<br/>节气+天气+场景"]
A --> C["核心分<br/>八字"]
A --> D["心愿分"]
A --> E["个性化加成<br/>历史偏好"]
A --> F["今日运势"]
...
**更新** 改进了梯度推荐策略，提供最佳匹配、保守替代和平衡方案三种类型，并在全部方案被一票否决时启用降级模式。
...
**章节来源**
- [weather.js](file://js/services/weather.js#L44-L85)
...
- [engine.js](file://js/services/engine.js#L155-L183)
...
- 权重异常：确认 getDynamicWeights 在无八字场景下的分支逻辑。
...
- 一票否决问题：检查 SCORING_WEIGHTS.threshold 中的阈值设置是否合理。
...
## 结论
评分引擎以"配置驱动 + 服务编排 + 核心评分器"为核心，结合五行理论与现代数据结构，实现了可解释、可扩展、可维护的推荐评分系统。通过动态权重、用户偏好与今日运势等机制，既保证了个性化，又兼顾了公平性与稳定性。新增的一票否决机制确保了推荐方案的基本质量标准，梯度推荐策略提供了更丰富的选择体验。未来可在缓存、分页与模板匹配等方面进一步优化，以支撑更大规模的数据与更复杂的业务场景。
...
**更新** 调整了权重分配，增加了八字权重，设置了门槛阈值。

**章节来源**
- [scoring-config.js](file://js/core/scoring-config.js#L6-L24)
...
**章节来源**
- [scoring-config.js](file://js/core/scoring-config.js#L20-L23)
...
**章节来源**
- [scoring-config.js](file://js/core/scoring-config.js#L119-L126)

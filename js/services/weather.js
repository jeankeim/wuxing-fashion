...
import { safeFetch, withErrorHandler, ErrorTypes } from '../core/error-handler.js';
...
export async function getCurrentWeather() {
  ...
  return getWeatherData(position.lat, position.lon);
}
 ...
 * @returns {string} 五行元素
 */
export function getWeatherElement(weatherType) {
  const elementMap = {
    sunny: 'fire',
    clear: 'fire',
    cloudy: 'metal',
  ...
  return elementMap[weatherType] || 'earth';
}
 ...
 * @returns {string} 五行元素
 */
export function getTemperatureElement(temperature) {
  if (temperature >= 30) return 'fire';
  if (temperature >= 25) return 'fire';
  if (temperature >= 15) return 'earth';
  if (temperature >= 5) return 'metal';
  return 'water';
}

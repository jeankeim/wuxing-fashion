...
export const SCENES = {
  ...
  WORK: { id: 'work', name: '职场', icon: '💼' },
  DATE: { id: 'date', name: '约会', icon: '💕' },
  PARTY: { id: 'party', name: '聚会', icon: '🎉' },
  SPORT: { id: 'sport', name: '运动', icon: '⚡' }
};
...
export function recordFeedback(schemeId, action, metadata = {}) {
  const feedback = safeStorage.getItem(STORAGE_KEY) || {};
  ...
  safeStorage.setItem(STORAGE_KEY, feedback);
...
function updateUserPreferences(schemeId, action, metadata) {
  const prefs = safeStorage.getItem(USER_PREFERENCES_KEY) || {
  ...
  safeStorage.setItem(USER_PREFERENCES_KEY, prefs);
}
...
export function getUserPreferences() {
  return safeStorage.getItem(USER_PREFERENCES_KEY) || {
    wuxingScores: { wood: 0, fire: 0, earth: 0, metal: 0, water: 0 },
    colorScores: {},
    materialScores: {},
    sceneScores: { daily: 0, work: 0, date: 0, party: 0, sport: 0 }
...
export function clearRecommendationData() {
  ...
  safeStorage.removeItem(USER_PREFERENCES_KEY);
}
